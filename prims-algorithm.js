...
var vertexNum, distanceMatrix, loop, loop2, loop3, startPoint, endPoint, distanceMatrixX, distanceMatrixY, edgeLength, inspectColumns = [], tempArray = [], tempSmallestNumber, temp1, temp2, alreadyDone1 = false, alreadyDone2 = false, alreadyDone3 = false, alreadyDone4 = false, alreadyDone5 = false, firstEdgeText = "", secondEdgeText = "", thirdEdgeText = "", fourthEdgeText = "", fifthEdgeText = "", totalEdgeLength = 0;
  ...
  if (vertexNum === 4) {
    ...
    "<tbody>"+
      ...
      "<tr>"+
        ...
        "<td>"+distanceMatrix[4][3]+"</td>"+
        "<td>"+distanceMatrix[4][4]+"</td>"+
        "<td>"+distanceMatrix[4][5]+"</td>"+
      "</tr>"+
      "<tr>"+
        "<td>F</td>"+
        "<td>"+distanceMatrix[5][0]+"</td>"+
        "<td>"+distanceMatrix[5][1]+"</td>"+
        "<td>"+distanceMatrix[5][2]+"</td>"+
        "<td>"+distanceMatrix[5][3]+"</td>"+
        "<td>"+distanceMatrix[5][4]+"</td>"+
        "<td>"+distanceMatrix[5][5]+"</td>"+
      "</tr>"+
    "</tbody>";
  } else {
    console.log("This program can only deal with graphs of 4-6 vertices.");
  }
}
...
function addEdge() {
  if (vertexNum === 4) {
    ...
    // console.log("Edge Added!");
    startPoint = prompt("Please enter your starting point.");
    if (startPoint !== "A" && startPoint !== "B" && startPoint !== "C" && startPoint !== "D" && startPoint !== "E" && startPoint !== "F") {
      // Invalid Response!  Terminate Process
      alert("Sorry, your response was invalid.  Please enter a capital letter: A/B/C/D/E/F");
    } else {
      if (startPoint === "A") {
        distanceMatrixX = 0;
      } else if (startPoint === "B") {
      ...
      endPoint = prompt("Please enter your endpoint.");
      if (endPoint !== "A" && endPoint !== "B" && endPoint !== "C" && endPoint !== "D" && endPoint !== "E" && endPoint !== "F") {
        alert("Sorry, your response was invalid.  Please enter a capital letter: A/B/C/D/E/F");
      } else {
        if (endPoint === startPoint) {
          alert("Invalid Request.  You cannot start and end at the same edge (unless you are deliberately trying to fool me :p)");
                                ...
                                if (typeof(distanceMatrix[loop2][inspectColumns[loop]]) === "number") {
                                  if (tempSmallestNumber !== distanceMatrix[loop2][inspectColumns[loop]] || alreadyDone4 === true) {
                                    ...
                                    for (loop = 0; loop < inspectColumns.length; loop++) {
                                      for (loop2 = 0; loop2 < vertexNum; loop2++) {
                                        if (typeof(distanceMatrix[loop2][inspectColumns[loop]]) === "number") {
                                          if (tempSmallestNumber !== distanceMatrix[loop2][inspectColumns[loop]] || alreadyDone5 === true) {
                                            console.log("Either this entry does not contain the smallest number or the smallest number has been found already.");
                                          } else {
                                            alreadyDone5 = true;
                                            // Mark down which edge was chosen
                                            ...
                                            document.getElementById("mainPanel").style.display = "none";
                                            document.getElementById("findMST").style.display = "block";
                                            document.getElementById("mstEdges").innerHTML = firstEdgeText + ", " + secondEdgeText + ", " + thirdEdgeText + ", " + fourthEdgeText + ", " + fifthEdgeText;
                                            document.getElementById("mstWeight").innerHTML = totalEdgeLength;
                                          }
                                        }
                                      }
